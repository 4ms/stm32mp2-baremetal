...
// Circular DMA helper, adapted from hal-audio/dma.hh but using 16-bit transfers
// since each ADC conversion result is one 16-bit halfword in the ADC DR register.
struct DmaHelper {
							   ...
							   DMA_NodeConfTypeDef &node_conf)
	{
		...
		// The DMA controller re-fetches the node from memory each time it loops,
		// so the node must be cleaned to memory. Note that HAL_ADC_Start_DMA()
		// re-writes the node's src/dst/size link registers (in cache only), so
		// the values passed to it must match node_conf, or the node must be
		// cleaned again after starting.
