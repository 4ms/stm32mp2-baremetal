	...
	switch (type) {
		...
		case USB_DT_STRING:
			...
			if (index < ARRAY_SIZE(string_table) && string_table[index]) {
				int len = make_string_desc(string_table[index], acm.ep0_req->buf, EP0_BUF_SIZE);
...
static int handle_setup(struct usb_gadget *gadget, const struct usb_ctrlrequest *ctrl)
{
	...
	if ((type & USB_TYPE_MASK) == USB_TYPE_CLASS) {
		switch (req) {
			case CDC_SET_LINE_CODING:
				...
				acm.ep0_req->length = min_t(int, wLength, sizeof(acm.line_coding));
				acm.ep0_req->complete = set_line_coding_complete;
				return usb_ep_queue(gadget->ep0, acm.ep0_req, 0);

			case CDC_GET_LINE_CODING:
				return ep0_reply(&acm.line_coding, sizeof(acm.line_coding), wLength);
