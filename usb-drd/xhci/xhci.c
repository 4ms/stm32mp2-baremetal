...
{
	/* Bare-metal: controller pointer stored directly on usb_device */
	return (struct xhci_ctrl *)udev->controller;
}
...
static int xhci_set_configuration(struct usb_device *udev)
{
	...
	/* Iterate over ALL interfaces to gather endpoint flags */
	for (int ifnum = 0; ifnum < udev->config.no_of_if; ifnum++) {
	...
	/* Fill endpoint contexts for ALL interfaces */
	for (int ifnum = 0; ifnum < udev->config.no_of_if; ifnum++) {
	ifdesc = &udev->config.if_desc[ifnum];
	num_of_ep = ifdesc->no_of_ep;
	for (cur_ep = 0; cur_ep < num_of_ep; cur_ep++) {
...
static int xhci_address_device(struct usb_device *udev, int root_portnr)
{
	...
	event = xhci_wait_for_event(ctrl, TRB_COMPLETION);
	if (!event) {
		printf("Address Device command timed out (device wedged?)\n");
		return -ETIMEDOUT;
	}
...
static int _xhci_alloc_device(struct usb_device *udev)
{
	...
	event = xhci_wait_for_event(ctrl, TRB_COMPLETION);
	if (!event) {
		printf("Enable Slot command timed out\n");
		return -ETIMEDOUT;
	}
		       ...
		       void *buffer, int length, struct devrequest *setup)
{
	...
	/* Walk up to find root hub port number */
	if (hop->parent) {
		while (hop->parent->parent)
			hop = hop->parent;
		root_portnr = hop->portnr;
	}
