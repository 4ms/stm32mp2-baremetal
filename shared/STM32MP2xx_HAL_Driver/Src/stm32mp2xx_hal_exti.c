...
/** @defgroup EXTI_Exported_Functions_Group4 Register Access control functions
 * @{
 */
...
/**
  ...
  * @param  Instance : pointer to exti structure
  * @param  ExtiLine : Exti line number.
...
{
  __IO uint32_t *regaddr, *regaddrP;
  ...
  regaddr = &Instance->ECIDCFGR[0] + (EXTI_LINE_CID_CFG_ADD_OFFSET * linepos);  /*selected register corresponding to current event*/
...
#if 0  //to update once API for locking RIF config will be froozen
/**
  * @brief  Configure RIF lock register. When set, RIF attribute of any EXTI line cannot be updated
  * @param  hexti : pointer to exti handler
  * @param  LockValue : SET_RIF_LOCK/CLEAR_RIF_LOCK

  * @retval void.
  */
void HAL_EXTI_ConfigLock(EXTI_HandleTypeDef *hexti, uint32_t LockValue)
{
  /* Check the parameters */
  assert_param(IS_RIF_LOCK(LockValue));

  hexti->Instance->LOCKR = LockValue;
}

/**
  * @brief  Get RIF lock attribute
  * @param  hexti : pointer to exti handler
  * @param  pLockValue : pointer of  RIF lock attribute.
  * @retval HAL Status.
  */
HAL_StatusTypeDef HAL_EXTI_GetConfigLock(EXTI_HandleTypeDef *hexti, uint32_t *pLockValue)
{
  /* Check null pointer */
  if (pLockValue == NULL)
  {
  ...
  *pLockValue = hexti->Instance->LOCKR;
  return HAL_OK;
}
#endif
/**
