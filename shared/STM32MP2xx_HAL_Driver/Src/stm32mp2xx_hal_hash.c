...
void HAL_HASH_ContextSaving(HASH_HandleTypeDef *hhash, uint32_t* pMemBuffer)
{
  ...
  pMemBuffer[CTX_SUSPEND_IMR] = READ_REG(HASH->IMR);

  /* Save STR register content */
  pMemBuffer[CTX_SUSPEND_STR] = READ_REG(HASH->STR);

  /* Save CR register content */
  pMemBuffer[CTX_SUSPEND_CR] = READ_REG(HASH->CR);
  /* Save SHA3CFG register content */
  pMemBuffer[CTX_SUSPEND_SHA3CFGR] = READ_REG(HASH->SHA3CFGR);

  /* By default, save all CSRs registers */
  for (unsigned int i = 0;i < HASH_NUMBER_OF_CSR_REGISTERS; i++)
  {
    pMemBuffer[CTX_SUSPEND_CSR+i] = READ_REG(HASH->CSR[i]);
  }
}
...
void HAL_HASH_ContextRestoring(HASH_HandleTypeDef *hhash, uint32_t* pMemBuffer)
{
  ...
  WRITE_REG(HASH->IMR,      pMemBuffer[CTX_SUSPEND_IMR]);

  /* Restore STR register content */
  WRITE_REG(HASH->STR,      pMemBuffer[CTX_SUSPEND_STR]);

  /* Restore CR register content */
  WRITE_REG(HASH->CR,       pMemBuffer[CTX_SUSPEND_CR]);

  /* Restore SHA3CFG register content */
  WRITE_REG(HASH->SHA3CFGR, pMemBuffer[CTX_SUSPEND_SHA3CFGR]);
  ...
  for (unsigned int i = 0;i < HASH_NUMBER_OF_CSR_REGISTERS; i++)
  {
    WRITE_REG(HASH->CSR[i], pMemBuffer[CTX_SUSPEND_CSR+i]);
  }
}
...
HAL_StatusTypeDef HAL_HASH_DMAFeed_ProcessSuspend(HASH_HandleTypeDef *hhash)
{
  ...
  else
  {
   ...
   /* Make sure there is enough time to suspend the processing */
    /* DMA3 used, DMA_CBR1_BNDT in byte*/
    tmp_remaining_DMATransferSize_inWords = ((((DMA_Channel_TypeDef *)hhash->hdmain->Instance)->CBR1) & DMA_CBR1_BNDT)/4;
...
static void HASH_MDMAXferCplt2(MDMA_HandleTypeDef *hmdma)
{
  ...
  if ((HASH->HWCFGR & 0xFF) >= 1)
  {
    ...
    datasentsize = (hhash->HashInCount%16 ? (hhash->HashInCount/16) *16:hhash->HashInCount - 16);
    remainsize = hhash->HashInCount - datasentsize;
    HASH_WriteData(hhash, hhash->pHashInBuffPtr + datasentsize , remainsize );
    if (!READ_BIT(HASH->CR, HASH_CR_MDMAT))
  ...
  if (hhash->State != HAL_HASH_STATE_SUSPENDED)
  {
    ...
    else
    {
      ...
      if (hhash->hmdmain != NULL)
      {
        ...
        HAL_MDMA_Start_IT(hhash->hmdmain, inputaddr, (uint32_t)&HASH->DIN,
              (buffersize%16 ? (buffersize/16) * 16:buffersize - 16), 1);
      }
...
static void HASH_DMAXferCplt(DMA_HandleTypeDef *hdma)
{
  ...
  if (hhash->State != HAL_HASH_STATE_SUSPENDED)
  {
    ...
    if (READ_BIT(HASH->CR, HASH_CR_DMAE) != RESET)
    {
      hhash->State = HAL_HASH_STATE_ERROR;
      return;
    }
...
static HAL_StatusTypeDef HASH_WriteData(HASH_HandleTypeDef *hhash, uint8_t *pInBuffer, uint32_t Size)
{
  ...
  {
    HASH_DriverTest_CheckAskSuspend_Poll(hhash, buffercounter);

    /* If a block is processed, enf of the processing  (occurs at each block+1 DIN writing) */
    while (__HAL_HASH_GET_FLAG(HASH_FLAG_BUSY)!=RESET);

    if ((hhash->SuspendRequest == HAL_HASH_SUSPEND)&&(__HAL_HASH_GET_FLAG(HASH_FLAG_DINIS)==SET))
    {
      /*if a  suspension is requested and if a partial hash is ready, function shall stop filling HASH buffer and save filled level value and exit*/
...
static HAL_StatusTypeDef HASH_WaitOnFlagUntilTimeout(HASH_HandleTypeDef *hhash, uint32_t Flag, FlagStatus Status, uint32_t Timeout)
{
  ...
  /* Wait until flag is different to Status */
  while (__HAL_HASH_GET_FLAG(Flag) == Status)
  {
...
static HAL_StatusTypeDef HASH_IT(HASH_HandleTypeDef *hhash)
{
  HASH_DriverTest_CheckAskSuspend_IT(hhash);
...
static uint32_t HASH_Write_Block_Data(HASH_HandleTypeDef *hhash)
{
  ...
  uint32_t nbByteToWriteToTriggPartialHash  = ( (HASH->SR & HASH_SR_NBWE_Msk) >> HASH_SR_NBWE_Pos) * 4;
...
{
  if(Algorithm == HASH_ALGOSELECTION_SHA1)         return 64;
  if(Algorithm == HASH_ALGOSELECTION_SHA224)       return 64;
  if(Algorithm == HASH_ALGOSELECTION_SHA256)       return 64;
  if(Algorithm == HASH_ALGOSELECTION_SHA384)       return 128;
  if(Algorithm == HASH_ALGOSELECTION_SHA512_224)   return 128;
  if(Algorithm == HASH_ALGOSELECTION_SHA512_256)   return 128;
  if(Algorithm == HASH_ALGOSELECTION_SHA512)       return 128;
  if(Algorithm == HASH_ALGOSELECTION_SHA3_224)     return 144;
  if(Algorithm == HASH_ALGOSELECTION_SHA3_256)     return 136;
  if(Algorithm == HASH_ALGOSELECTION_SHA3_384)     return 104;
  if(Algorithm == HASH_ALGOSELECTION_SHA3_512)     return 72;
  if(Algorithm == HASH_ALGOSELECTION_SHAKE_128)    return 168;
  if(Algorithm == HASH_ALGOSELECTION_SHAKE_256)    return 136;
  if(Algorithm == HASH_ALGOSELECTION_RAWSHAKE_128) return 168;
  if(Algorithm == HASH_ALGOSELECTION_RAWSHAKE_256) return 136;
  return 0;
}
...
HAL_StatusTypeDef HASH_Start_DMA(HASH_HandleTypeDef *hhash, uint8_t *pInBuffer, uint32_t Size, uint32_t Algorithm)
{
  ...
  if((State_tmp == HAL_HASH_STATE_READY) || (State_tmp == HAL_HASH_STATE_SUSPENDED))
  {
    ...
    if (hhash->hmdmain != NULL)
    {
      ...
      /* Enable the DMA In DMA Stream */
      if (READ_BIT(HASH->CR, HASH_CR_MDMAT))
        status = HAL_MDMA_Start_IT(hhash->hmdmain, inputaddr, (uint32_t)&HASH->DIN, inputSize, 1);
      else
        status = HAL_MDMA_Start_IT(hhash->hmdmain, inputaddr, (uint32_t)&HASH->DIN,
                  (inputSize%16 ? ((inputSize/16)*16): inputSize - 16), 1);
    }
#endif
...
/**
  * @brief  Check if suspend request should be set in polling mode (Driver test purpose)
  * @param  hhash: HASH handle.
  * @param  buffercounter : number of bytes pushed in HASH
  * @retval None
  */
__weak void HASH_DriverTest_CheckAskSuspend_Poll(HASH_HandleTypeDef *hhash, uint32_t buffercounter)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhash);
  UNUSED(buffercounter);

  /* NOTE : This function should not be modified; when the callback is needed,
            HASH_DriverTest_CheckAskSuspend_Poll() can be implemented in the user file.
   */
}

/**
  * @brief  Check if suspend request should be set in IT mode (Driver test purpose)
  * @param  hhash: HASH handle.
  * @param  buffercounter : number of bytes pushed in HASH
  * @retval None
  */
__weak void HASH_DriverTest_CheckAskSuspend_IT(HASH_HandleTypeDef *hhash)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hhash);

  /* NOTE : This function should not be modified; when the callback is needed,
            HASH_DriverTest_CheckAskSuspend_IT() can be implemented in the user file.
   */
}
