...
void UsbSerialDevice::start()
{
	...
	if (init_ok != USBD_OK) {
		printf("USB Serial Device failed to initialize!\r\n");
...
{
	printf("Stopping UsbSerialDevice\n");
	USBD_Stop(pdev);
	USBD_DeInit(pdev);
}
	...
	auto transmit = [this, dest = dest](uint8_t *ptr, int len) {
		if (dest == Destination::USB) {
			...
			if (auto err = USBD_CDC_TransmitPacket(pdev) == USBD_OK) {
				...
				printf("USB CDC Transmit Error: %d\n", err);
			}
		}
...
{
	USBD_CDC_SetTxBuffer(_instance->pdev, _instance->console_buffer.data(), 0);
